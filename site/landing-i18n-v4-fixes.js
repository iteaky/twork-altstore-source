(() => {
  ...
  const translateString = (source,language) => {
    ...
    if (language === 'en' && exactEnglish[source.trim()]) return exactEnglish[source.trim()];
    ...
    // Never fake a translation by romanising Cyrillic. Unknown text stays intact
    // until it receives a real dictionary entry.
    return result;
  };
